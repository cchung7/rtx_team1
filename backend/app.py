...
# Initialize database (optional - falls back to CSV if unavailable)
# Database functionality removed - using CSV files instead

# Initialize CSV data source
try:
    data_source = get_data_source(data_path='../data/')
    logger.info(f"CSV data source loaded: {len(data_source.df)} records")
except Exception as e:
    logger.error(f"Failed to load CSV data source: {str(e)}")
    data_source = None
    ...
    try:
        ...
        if model_name == 'prototype':
            ...
            if os.path.exists(model_path):
                ...
                logger.info(f"{model_name.title()} model loaded successfully")
            else:
            ...
            if os.path.exists(model_path):
                ...
                logger.warning(f"Balanced model not found at {model_path}")
        else:
            ...
            if predictor.load_model():
                ...
                logger.warning(f"No {model_name} model found")
    except Exception as e:
...
def predict_aqi():
    ...
    Body: { "county": "string", "state": "string", "model": "string" }
    """
    try:
        ...
        # Get recent data for county to create features using CSV
        if data_source is None:
            return jsonify({
                'success': False,
                'error': 'Data source not available'
        ...
        if model_type == 'prototype':
            ...
            X_forecast = np.array([[
                ...
                0,  # Defining Parameter_CO (not available in daily data)
                0,  # Defining Parameter_NO2 (not available in daily data)
                0,  # Defining Parameter_Ozone (not available in daily data)
                0,  # Defining Parameter_PM10 (not available in daily data)
                0,  # Defining Parameter_PM2.5 (not available in daily data)
                features['aqi_lag_1'],  # AQI_lag1
                features['aqi_lag_3'],  # AQI_lag3
                features['aqi_lag_7']   # AQI_lag7
            ]])
        elif model_type == 'balanced':
            # Balanced model expects 11 features: State Code, County Code, 4 lag features, 2 rolling, 2 temporal, 1 volatility
            county_data = recent_df.iloc[0]
            ...
            X_forecast = np.array([[
                ...
                features['aqi_lag_3'],  # AQI_lag3
                features['aqi_lag_7'],  # AQI_lag7
                features.get('aqi_lag_14', features['aqi_lag_7']),  # AQI_lag14 (if available, else use lag_7)
                features['aqi_rolling_7'],  # AQI_rolling_7
            ...
            X_forecast = np.array([[
                ...
                features['aqi_rolling_14'],
                features['aqi_rolling_30']
            ]])
            
            # Pad with zeros to match training feature count if needed
        ...
        # Scale features for the selected model
        if model_type == 'prototype':
            ...
            try:
                ...
                logger.error(f"Failed to load prototype scaler: {str(e)}")
                return jsonify({
                    'success': False,
                    'error': 'Failed to load prototype model scaler'
                }), 500
        elif model_type == 'balanced':
            ...
            try:
                ...
                logger.error(f"Failed to load balanced scaler: {str(e)}")
                return jsonify({
                    'success': False,
                    'error': 'Failed to load balanced model scaler'
                }), 500
        else:
        ...
        for day in range(days):
            ...
            if model_type == 'prototype':
                ...
                # This shouldn't happen anymore since we removed improved model
            ...
            day_prediction = selected_predictor.forecast(
                ...
                store_predictions=False  # Don't store intermediate predictions
            )
            ...
            if 'aqi_rolling_7' in current_features:
                # Approximate rolling average update
...
def refresh_data():
    ...
    try:
        ...
        logger.info(f"Refreshing data for {county}, {state}")
...
if __name__ == '__main__':
    logger.info("="*60)
    logger.info("Starting CLAP Flask API Server")
    logger.info("="*60)
